...
Factors are the numbers less than the input number that can be evenly divided into it. For example, the 
number 6 can be evenly divided by 1, 2, and 3.

# Examples
From the test cases shown above, we can see that we need to create a PerfectNumber class. The class should 
have 1 method:

It accepts a number as an argument and returns the appropriate string based on its classification: 
...
# Data 
In addition to working with numbers, it may also be helpful to use a collection that can hold a range. 
This could prove useful in finding all of the divisors of a given number.
...
A number is a divisor of another number if the remainder of dividing the second number by the first is 0. 
For example, 7 is a divisor of 21 since 21 % 7 is 0. However, 7 is not a divisor of 23 since 23 % 7 is not 
0 -- it is 2.
...
In our example cases, we could see that we needed to create a class method, so no constructor was needed. 
First, we make sure to raise an error if the input is negative. Then, we use our helper method 
sum_of_factors to calculate the sum of all the divisors.

Within the helper method, we iterate over 1 up to one less than the given number. We select numbers from 
this range that are even divisors using the modulo, and then we add all of the selected numbers using sum.

Note that our sum_of_factors helper method is a class (or static) method. We'd like the method to be 
private since it's of no use to the users of our class. Unfortunately, this code won't work as expected:
...
Even though it looks like we're creating a private method, the private method doesn't work on class 
methods -- it only works on instance methods. Fortunately, there is a way around that using the 
class << self idiom lets us operate on the class itself as an object; by adding a private instance 
method to the class object, we effectively create a private class method. Why this works isn't important 
at this stage, but it's a useful idiom to know.
